...
5. Whole-line live view (Cris, banked 2026-08-17): a collapsed card that opens into a vertical strip of the entire G Line, west end (63rd) at the top, east end (Richmond-Westmoreland) at the bottom, one pulsing green dot per PCC car at its current position with an up or down arrow for its direction (up is westbound, down is eastbound). One glance shows the whole line. Data already exists: `currentSequence`, `direction`, and the stop list in app.js.
6. "New trolley in service" alert (Cris, banked 2026-08-17): a push when a car that has not been seen before appears on the G Line (a car number not in the roster of logged PCC IDs). Fits with item 1's unknown-ID alert; the roster comes from pcc_observations. Ask Cris whether "new" means new to the app's history or new today (the "each new car" alert already covers today).
7. Later: per-vehicle reliability views once months of data exist (compare each vehicle's lateness against other vehicles on the same block and hour; SEPTA on-time is 1 minute early to 6 late at timepoints; SEPTA publishes route-level OTP only).
